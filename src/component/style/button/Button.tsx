...
export const ButtonDefault: FC<ButtonDefaultProps> = (props: ButtonDefaultProps) => {
  return <button type={props.type} disabled={props.disabled} onClick={props.onClick} className={`p-5 disabled:cursor-not-allowed disabled:opacity-50 rounded w-full ease-out duration-300 py-3 px-4 ${props.extraClass}`}>{props.children}</button>;
};
